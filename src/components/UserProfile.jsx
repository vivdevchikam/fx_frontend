...
const UserProfile = ({ isOpen, onClose, currentPlan = "free" }) => {
        ...
        <div className="relative bg-gradient-to-r from-emerald-500 to-sky-500 p-6">
          ...
          <div className="grid grid-cols-3 gap-4 mt-6">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-white">
                {(
                  scanHistory.reduce((sum, scan) => sum + scan.score, 0) /
                  scanHistory.length
                ).toFixed(1)}
              </div>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 text-center">
              ...
              <div className="text-white/80 text-sm">
                ...
                {currentPlan === "free" && (
                  <div className="text-xs text-white/60 mt-1">
                    <span className="text-yellow-300">🔒</span> Advanced
                    tracking
                  </div>
                )}
              </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {healthOptions.map((option) => (
                    <button
                      key={option.id}
                      onClick={() =>
                        isEditing && handleHealthGoalToggle(option.label)
                      }
                      disabled={!isEditing}
          ...
          {activeTab === "history" && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {scanHistory
                  .slice(0, currentPlan === "free" ? 3 : scanHistory.length)
                  .map((scan) => (
                    <div
                      key={scan.id}
                ...
                {currentPlan === "free" && scanHistory.length > 3 && (
                  <div className="text-center py-8 bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl border border-purple-200">
                    <p className="text-purple-800 font-medium mb-2">
                      🔒 {scanHistory.length - 3} more scans available with
                      Premium
                    </p>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Get reminded to scan your food daily
                      </p>
                    </div>
                    <button
                      onClick={() => handleNotificationToggle("dailyReminders")}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        profileData.notifications.dailyReminders
                          ? "bg-emerald-500"
                          : "bg-gray-300"
                      }`}
                    >
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Receive weekly summaries of your health choices
                      </p>
                    </div>
                    <button
                      onClick={() => handleNotificationToggle("weeklyReports")}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        profileData.notifications.weeklyReports
                          ? "bg-emerald-500"
                          : "bg-gray-300"
                      }`}
                    >
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Get notified about concerning ingredients
                      </p>
                    </div>
                    <button
                      onClick={() => handleNotificationToggle("healthAlerts")}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        profileData.notifications.healthAlerts
                          ? "bg-emerald-500"
                          : "bg-gray-300"
                      }`}
                    >
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Be the first to know about new FX Health features
                      </p>
                    </div>
                    <button
                      onClick={() => handleNotificationToggle("newFeatures")}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        profileData.notifications.newFeatures
                          ? "bg-emerald-500"
                          : "bg-gray-300"
                      }`}
                    >
              ...
              <div className="bg-gray-50 rounded-2xl p-6">
                ...
                <div className="space-y-3">
                  <button className="w-full text-left p-3 bg-white rounded-xl border border-gray-200 hover:border-emerald-300 transition-colors">
                    ...
                    <p className="text-sm text-gray-500">
                      Download all your scan history and preferences
                    </p>
                  </button>
                  <button className="w-full text-left p-3 bg-white rounded-xl border border-gray-200 hover:border-red-300 transition-colors">
                    ...
                    <p className="text-sm text-gray-500">
                      Sign out of your FX Health account
                    </p>
                  </button>
                </div>
              </div>
