...
import { 
  Newspaper, Clock, TrendingUp, Heart, Brain, Shield, 
  Apple, AlertTriangle, ExternalLink, BookOpen, Star,
  Filter, Search, ChevronRight, Tag, Calendar, User
} from 'lucide-react';
...
interface HealthNewsProps {
  context?: 'scan-result' | 'homepage' | 'standalone';
...
const HealthNews: React.FC<HealthNewsProps> = ({ 
  context = 'standalone', 
  scannedIngredients = [], 
  productCategory = '',
  userHealthGoals = [],
  ...
  const mockArticles: NewsArticle[] = [
    ...
    {
      id: '3',
      title: 'Organic vs. Conventional: New Study Reveals Nutritional Differences',
      summary: 'Comprehensive analysis shows organic foods contain 20-40% higher levels of antioxidants compared to conventional alternatives.',
      content: 'The largest meta-analysis to date examining organic versus conventional foods has revealed significant nutritional differences...',
      category: 'organic',
    ...
    },
    {
      ...
      summary: 'Reducing sodium intake by just 1000mg daily can lower blood pressure and reduce heart disease risk by 15%.',
      content: 'New cardiovascular research demonstrates that modest reductions in daily sodium consumption can have profound health benefits...',
      category: 'heart-health',
      tags: ['sodium', 'blood-pressure', 'heart-disease', 'cardiovascular'],
      publishedAt: '2024-01-12T16:45:00Z',
      readTime: 3,
      source: 'Cardiology News',
      imageUrl: 'https://images.pexels.com/photos/6182527/pexels-photo-6182527.jpeg?auto=compress&cs=tinysrgb&w=400',
      relevanceScore: 7.9
    },
    {
      id: '5',
      title: 'Protein Quality: Complete vs. Incomplete Proteins Explained',
      summary: 'Understanding amino acid profiles can help you make better protein choices for muscle health and recovery.',
      content: 'Nutritionists break down the science behind protein quality and how to combine foods for optimal amino acid intake...',
      category: 'fitness',
    ...
    },
    {
      ...
      summary: 'Latest research examines the safety profile of popular artificial sweeteners and their impact on metabolism.',
      content: 'A balanced look at current evidence surrounding artificial sweeteners, including aspartame, sucralose, and stevia...',
      category: 'ingredients',
      tags: ['artificial-sweeteners', 'aspartame', 'sucralose', 'stevia', 'metabolism'],
    ...
    if (context === 'scan-result' && scannedIngredients.length > 0) {
      filtered = filtered.filter(article =>
        article.tags.some(tag =>
          scannedIngredients.some(ingredient =>
  ...
  return (
    <section className={`${context === 'scan-result' ? 'py-8' : 'py-20'} bg-gradient-to-br from-gray-50 to-white`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <h2 className={`${context === 'scan-result' ? 'text-2xl' : 'text-4xl'} font-bold text-gray-900 mb-4`}>
            {getContextualTitle()}
          </h2>
          <p className={`${context === 'scan-result' ? 'text-lg' : 'text-xl'} text-gray-600 max-w-3xl mx-auto`}>
        ...
        {context === 'scan-result' && scannedIngredients.length > 0 && (
          <div className="mb-8 text-center">
            <p className="text-sm text-gray-600 mb-3">News related to ingredients in your product:</p>
        ...
        {/* Articles Grid */}
        <div className={`grid ${context === 'scan-result' ? 'md:grid-cols-2' : 'md:grid-cols-2 lg:grid-cols-3'} gap-8`}>
          {filteredArticles.slice(0, context === 'scan-result' ? 4 : 9).map((article, index) => (
            <article
              key={article.id}
        ...
        {context === 'standalone' && (
          <div className="mt-20 bg-gradient-to-r from-emerald-500 to-sky-500 rounded-3xl p-12 text-white text-center">
            <h3 className="text-3xl font-bold mb-4">Stay Updated with Health News</h3>
